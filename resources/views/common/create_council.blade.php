<div class="row">
							...
							<tbody>
								...
								@if (!empty($topic_edit))
									@foreach ($topic_edit AS $topic)
										<tr>
											<td>
												<input name="choose[]" value="{{ $topic->pk_imadt }}" class="iCheck" type="checkbox" checked="checked"{{ (!empty($array[$topic->pk_imadt]) && $array[$topic->pk_imadt] == $topic->pk_imadt) ? 'checked' : null }}>
											</td>
											<td>{{ $topic->smasodetai }}</td>
											<td>{{ $topic->stendt }}</td>
										</tr>
									@endforeach
								@endif
								...
								@if (count($data['topic_list']) > 0)
									...
									<tr>
										<td class="text-center">
											<input name="choose[]" value="{{ $topic->pk_imadt }}" class="iCheck" type="checkbox" {{ (!empty($array[$topic->pk_imadt]) && $array[$topic->pk_imadt] == $topic->pk_imadt) ? 'checked' : null }}>
										</td>
										<td>{{ $topic->smasodetai }}</td>
	                            		<td>{{ $topic->stendt }}</td>
	                            	</tr>
	...
	<div class="col-sm-6">
		<section class="content">
	        <div class="box box-info">
	            <div class="box-header with-border">
	                <h3 class="box-title">{{ $council_infomation }}</h3>
	            </div><!--/.box-header-->
